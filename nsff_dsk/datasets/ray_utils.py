...
def get_ray_directions(H, W, K, D=None, return_uv=False, flatten=True):
    """
    先用meshgrid生成图像物理坐标系下的各个图像点，然后通过相机内参变换得到相机坐标系下的对应点(仍为矩形，z轴距离统一为1)
    ...
    # 图像物理坐标系-->相机坐标系(directions)
    directions = \
        torch.stack([(i-cx)/fx, -(j-cy)/fy, -torch.ones_like(i)], -1)
    if flatten:
        directions = directions.reshape(-1, 3)     # 相机坐标系展平(num_of_point, 3)
        grid = grid.reshape(-1, 2)                 # 图像物理坐标系展平(num_of_point, 2)
