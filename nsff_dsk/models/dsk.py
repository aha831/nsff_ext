...
class DSKnet(nn.Module):
    ...
    def forward(self, uv, img_idx):
        ...
        # delta_trans预测的是相机源点在相机坐标系下xy两个方向上的偏移量
        # delta_pos预测的是模糊核的5个点各自位置在图像坐标系下的xy偏移量
        delta_trans = delta_trans * 0.01
        # 实际上模糊核的5个偏移量是由dsk计算的偏移预测量与各自的初始位置参数(可学习)相加得到的
        ...
        # 射线源点的位置偏移量只预测相机坐标系下在xy两个方向上的变化，即对o的偏移建模不涉及深度方向
