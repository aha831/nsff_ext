...
from models.run_nerf_helpers import visualize_kernel
...
def render_rays(models,
    ...
    Inputs:
        ...
        is_train: 是否是训练阶段, 是的话则使用dsk_net(deprecated), 通过img_idx是否为None来判断

        **kwargs: 其他参数，包括'view_dir'('max frequency in dir embedding')、't_embedded'、'a_embedded'
    Outputs:
    ...
    def inference(results, model, xyz, zs, test_time=False, **kwargs):
        ...
        if typ=='coarse' and test_time:
            ...
            out = torch.cat(out_chunks, 0)
            out = rearrange(out, '(n1 n2) c -> n1 n2 c', n1=N_rays, n2=N_samples_)   # c = 14
        ...
        if output_transient:
            ...
            if (not test_time) and output_transient_flow: # render with flowed-xyzs
                ...
                # to compute fw-bw consistency, 理论上二者都应该等于xyzs
    ...
    def inference_blur(results, model, xyz_blur, zs_blur, deblur_weight):
        """
        Helper function that performs model inference.
        Inputs:
            results: a dict storing all results
        ...
        typ = model.typ
        # results中也记录了输入的情况
        N_samples_ = xyz_blur.shape[1]
        xyz_ = rearrange(xyz_blur, 'n1 n2 c -> (n1 n2) c', c=3)

        # Perform model inference to get rgb and raw sigma
        B = xyz_.shape[0]
        out_chunks = []
        
        dir_embedded_ = repeat(dir_embedded_blur, 'n1 c -> (n1 n2) c', n2=N_samples_)
        if model.encode_appearance:
            a_embedded_ = repeat(a_embedded, 'n1 c -> (n1 n2) c', n2=N_samples_)
        if output_transient:
            t_embedded_blur_ = repeat(t_embedded_blur, 'n1 c -> (n1 n2) c', n2=N_samples_)
        for i in range(0, B, chunk):
            inputs = [embedding_xyz(xyz_[i:i+chunk]), dir_embedded_[i:i+chunk]]
            if model.encode_appearance: 
                inputs += [a_embedded_[i:i+chunk]]
            if output_transient: inputs += [t_embedded_blur_[i:i+chunk]]
            out_chunks += [model(torch.cat(inputs, 1),
                                    output_transient=output_transient)]

        out = torch.cat(out_chunks, 0)
        out = rearrange(out, '(n1 n2 n3) c -> n1 n2 n3 c', n1=N_rays, n2=5, n3=N_samples_)   # c = 14
        static_rgbs = out[..., :3]
        static_sigmas = out[..., 3]
        if output_transient:
        ...
        deltas = zs_blur[..., 1:] - zs_blur[..., :-1] # (N_rays, N_samples_-1)
        static_deltas = torch.cat([deltas, 100*torch.ones_like(deltas[..., :1])], -1)
        transient_deltas = torch.cat([deltas, 1e-3*torch.ones_like(deltas[..., :1])], -1)
        ...
        if output_transient:
            static_alphas = alphas
            transient_sigmas = act(transient_sigmas+torch.randn_like(transient_sigmas)*noise_std)
            transient_alphas = 1-torch.exp(-transient_deltas*transient_sigmas)
            alphas = 1-(1-static_alphas)*(1-transient_alphas)
        ...
        weights = alphas * transmittance # (N_rays, N_samples_)
        weights_ = rearrange(weights, 'n1 n2 n3-> n1 n2 n3 1')

        if output_transient:
            static_rgb_map = reduce(rearrange(static_weights, 'n1 n2 n3 -> n1 n2 n3 1')*static_rgbs,
                                    'n1 n2 n3 c -> n1 n2 c', 'sum')
            transient_weights_ = rearrange(transient_weights, 'n1 n2 n3 -> n1 n2 n3 1')
            transient_rgb_map = reduce(transient_weights_*transient_rgbs, 'n1 n2 n3 c -> n1 n2 c', 'sum')
            deblur_rgb = static_rgb_map + transient_rgb_map
            # deblur_rgb = reduce(deblur_rgb, 'n1 n2 c -> n1 c', 'mean')
            deblur_rgb = torch.sum(deblur_rgb * deblur_weight[..., None], dim=1)
            results[f'rgb_{typ}'] = deblur_rgb
...
###################################################
    ...
    if img_idx is not None:
        new_rays, deblur_weight, align_loss = dsk_net(rays[..., -2:], img_idx)
        # visualize_kernel(dsk_net, uv=rays[..., -2:], img_idx=img_idx, color=True, weight=weight)
        rays_o_blur = new_rays[..., 0].reshape(-1, 3)
        rays_d_blur = new_rays[..., 1].reshape(-1, 3)
        rays_o_blur = rays_o_blur.to(dtype=torch.float32)
        rays_d_blur = rays_d_blur.to(dtype=torch.float32)
        dir_embedded_blur = embedding_dir(kwargs.get('view_dir', rays_d_blur))
        rays_o_blur = rearrange(rays_o_blur, 'n1 c -> n1 1 c')
        rays_d_blur = rearrange(rays_d_blur, 'n1 c -> n1 1 c')
    ...
    z_far = 0.95 # explicitly zero the flow if z exceeds this value  超过这个深度的像素点不考虑其光流值
    ...
    if N_importance == 0:
        ...
        if output_transient:
            t_embedded = kwargs['t_embedded'] if 't_embedded' in kwargs else embeddings['t'](ts)
            if img_idx is not None:
                t_embedded_blur = t_embedded[:,None,:].repeat(1, 5, 1).reshape(-1, t_embedded.size(-1))
    ...
    if img_idx is not None:
        zs_blur = rearrange(zs, 'n1 n2 -> n1 1 n2')
        zs_blur = zs_blur.expand(zs.size(0), dsk_net.num_pt, zs.size(1))
        zs_blur = zs_blur.reshape(-1, zs.size(1))
        xyz_blur = rays_o_blur + rays_d_blur * rearrange(zs_blur, 'n1 n2 -> n1 n2 1')
        xyz_blur = xyz_blur.to(dtype=torch.float32)
        zs_blur = rearrange(zs_blur, '(n1 n2) c -> n1 n2 c', n1=N_rays, n2=5)
        inference_blur(results, model, xyz_blur, zs_blur, deblur_weight)
        # 添加dsk_net时暂时没有考虑course的情况
        deblur_loss = align_loss

    return results, deblur_loss
