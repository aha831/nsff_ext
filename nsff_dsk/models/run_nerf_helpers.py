...
@torch.no_grad()
def visualize_kernel(kernelsnet, uv, img_idx=1, x=1, y=1, depth=0.5, color=False, weight=False):
    if isinstance(kernelsnet, nn.DataParallel):
        kernelsnet = kernelsnet.module

    kernelsnet.cuda()

    uv_show = uv.clone()
    # ray_info["images_idx"] = torch.tensor(img_idx).type(torch.int64).cuda().reshape(-1, 1).expand(100, 1)
    uv_show[..., 0] = torch.ones_like(uv_show[..., 0]) * x
    uv_show[..., 1] = torch.ones_like(uv_show[..., 1]) * y
    kernelsnet.random_hwindow = 0
    rays, weights, _ = kernelsnet(uv_show, img_idx)
    rays_d = rays[..., 1]
    poses = kernelsnet.poses
    r_inv = poses[img_idx:img_idx+1, :3, :3].inverse()
    rays_d = (r_inv[:, None] @ rays_d[..., None]).squeeze(-1)
    rays_d = rays_d / -rays_d[..., -1:]
    rays_x = rays_d[..., 0] * kernelsnet.K[0, 0] + kernelsnet.K[0, 2]
    rays_y = rays_d[..., 1] * kernelsnet.K[1, 1] - kernelsnet.K[1, 2]  # it's reversed, attention!
    rays_x = rays_x - uv_show[..., 0:1]
    rays_y = rays_y + uv_show[..., 1:]
