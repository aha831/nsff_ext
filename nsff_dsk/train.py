...
class NSFFSystem(LightningModule):
    def __init__(self, hparams):
        ...
        self.DSKnet = DSKnet(self.N_frames)  # 读取数据后将c2w矩阵赋给DSKnet.poses
    ...
    def forward(self, rays, ts, img_index, test_time=False, **kwargs):
        ...
        for i in range(0, B, self.hparams.chunk):
            rendered_ray_chunks, deblur_loss = \
        ...
        if deblur_loss is not None:
            results['deblur_loss'] = deblur_loss
        return results
    ...
    def setup(self, stage):
        ...
        poses = self.train_dataset.poses
        bottom = np.zeros((self.N_frames, 1, 4))
        bottom[..., -1] = 1
        poses = torch.tensor(np.linalg.inv(np.concatenate([poses, bottom], 1)))
        self.DSKnet.poses = poses
        self.DSKnet.K = torch.tensor(self.train_dataset.K)
        self.DSKnet.H = self.train_dataset.img_wh[0]
        self.DSKnet.W = self.train_dataset.img_wh[1]
    ...
    def validation_step(self, batch, batch_nb):
        ...
        img_index = None  # 不使用dsk_net
        results = self(rays, ts, img_index, test_time=True, **kwargs)
