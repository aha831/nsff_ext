...
from datasets.ray_utils import ndc2world
...
class NSFFSystem(LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        ...
        flow_th = 5
        xyzs_flow_ndc = results['xyzs_fw'] + results['xyzs_fw']
        point_nums = xyzs_flow_ndc.size(0)
        xyzs_flow_ndc = xyzs_flow_ndc.reshape(-1, 3)
        flow_K = torch.tensor(self.train_dataset.K).to(xyzs_flow_ndc.device)
        xyzs_flow_world = ndc2world(xyzs_flow_ndc, flow_K)
        xyzs_flow_world = xyzs_flow_world.reshape(point_nums, -1, 3)
        xyzs_flow = torch.pow(torch.pow(xyzs_flow_world[..., 0], 2) + \
                  torch.pow(xyzs_flow_world[..., 1], 2) + \
                  torch.pow(xyzs_flow_world[..., 2], 2), 0.5)

        zs_far = torch.ge(results['zs_fine'], 0.95)
        xyzs_flow[zs_far] = 0.0
        static_flag = torch.lt(xyzs_flow, flow_th)
        
        loss_trans_flow = torch.mean(results['transient_sigmas_fine'][static_flag])
        loss_trans_flow *= 1e-2

        loss_d = self.loss(results, batch, **kwargs)
        loss = sum(l for l in loss_d.values())
        loss += loss_trans_flow
